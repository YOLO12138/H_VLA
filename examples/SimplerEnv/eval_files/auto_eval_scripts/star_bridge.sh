...
echo `which python`
# Environment setup
cd /mnt/petrelfs/yejinhui/Projects/hvla
export hvla_python=/mnt/petrelfs/share/yejinhui/Envs/miniconda3/envs/hvla/bin/python
...
MODEL_PATH=$1
# MODEL_PATH=/mnt/petrelfs/yejinhui/Projects/hvla/results/Checkpoints/1120_bridge_rt_1_QwenDual_florence/checkpoints/steps_11000_pytorch_model.pt
TSET_NUM=4 # repeat each task 4 times
run_count=0

if [ -z "$MODEL_PATH" ]; then
  echo "❌ MODEL_PATH not provided as the first argument; using default"
  export MODEL_PATH="/mnt/petrelfs/yejinhui/Projects/hvla/results/Checkpoints/1007_qwenLargefm/checkpoints/steps_20000_pytorch_model.pt"
fi
...
start_service() {
  ...
  CUDA_VISIBLE_DEVICES=${gpu_id} ${hvla_python} deployment/model_server/server_policy.py \
    --ckpt_path ${ckpt_path} \
    --port ${port} \
    --use_bf16 \
    > "${svc_log}" 2>&1 &
