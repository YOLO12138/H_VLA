...
2. Running the evaluation by launching services in both `hvla` and `simpler_env` environments.  
...
- **hvla environment**: runs the policy inference server.  
...
### Step 1. Start the server (hvla environment)

In the first terminal, activate the `hvla` conda environment and run:  
...
```bash
python hvla/dataloader/lerobot_datasets.py --config_yaml examples/SimplerEnv/train_files/hvla_cotrain_oxe.yaml
```
...
```bash
python hvla/model/framework/QwenGR00T.py --config_yaml examples/SimplerEnv/train_files/hvla_cotrain_oxe.yaml
```
