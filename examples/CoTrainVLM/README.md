...
## HVLA Co-Training Guide: VLA with VLM Data 🚀

This guide outlines the process for integrating VLM (Vision-Language Model) data to co-train the HVLA (Vision-Language-Action) framework, enhancing its general visual and language understanding.
...
Register your dataset by adding it to the `data_dict` in [qwen_data_config.py](./hvla/dataloader/qwenvl_llavajson/qwen_data_config.py#L17).
...
```bash
python hvla/dataloader/vlm_datasets.py --config_yaml your_train_config.yaml
```
...
Use this for VLM-specific pre-training or fine-tuning.
  ...
  * **Script:** `hvla/training/train_starvlm.py`
...
This simultaneously trains the model on both robotics (VLA) and multi-modal (VLM) data.
  ...
  * **Script:** `hvla/training/train_hvla_cotrain.py`
