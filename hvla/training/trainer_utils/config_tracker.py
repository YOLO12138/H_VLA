...
_original_select = OmegaConf.select
...
def _patched_select(cfg, key, *, default=None, throw_on_missing=False):
    """Patched OmegaConf.select that handles AccessTrackedConfig"""
    if isinstance(cfg, AccessTrackedConfig):
        cfg = cfg.unwrap()
    return _original_select(cfg, key, default=default, throw_on_missing=throw_on_missing)
...
OmegaConf.select = _patched_select