...
class DiTX(nn.Module):
    ...
    def __init__(
        ...
        # Dynamic time adaptor input dim: timestep + target_t (always)
        # + state_emb (if state_injection_mode == "timestep_concat")
        # + lang_emb  (if language_conditioned and lang_injection_mode == "timestep_concat")
        n_time_inputs = 2
        ...
        if self.language_conditioned:
            ...
            # pre-norm language modality (only used in "token" mode)
            if self.pre_norm_modality and self.lang_injection_mode == "token":
        ...
        if block_type == "DiTX":
            self.blocks = nn.ModuleList(
                [
                    DiTXBlock(
                        n_emb, n_head, mlp_ratio=mlp_ratio, p_drop_attn=p_drop_attn, qkv_bias=qkv_bias, qk_norm=qk_norm
                    )
    ...
    def forward(
        ...
        **kwargs,
        ...
        Input:
            ...
            state_emb: (B, n_emb) optional; when state_injection_mode=="timestep_concat", concatenated with time embedding
        ...
        # Time conditioning: build time_parts dynamically
        time_parts = [timestep_embed, target_t_embed]
        if self.state_injection_mode == "timestep_concat":
            if state_emb is None:
                raise ValueError(
                    "state_injection_mode is 'timestep_concat' but state_emb is None. "
                    "Ensure batch['obs'] contains 'agent_pos' (dataset include_state=True "
                    "and collate adds obs['agent_pos'])."
                )
            time_parts.append(state_emb)
        if self.language_conditioned and self.lang_injection_mode == "timestep_concat":
            ...
            lang_sent = self.lang_adaptor(lang_sent)  # (B, n_emb)
            time_parts.append(lang_sent)
        time_input = torch.cat(time_parts, dim=-1)
        time_c = self.timestep_target_t_adaptor(time_input)  # (B, n_emb)
        ...
        # 4. language condition (token mode: concatenate on sequence dim)
        if self.language_conditioned and self.lang_injection_mode == "token":
            ...
            lang_c = self.lang_adaptor(lang_c)  # (B, L_lang, n_emb)
            if self.pre_norm_modality:
