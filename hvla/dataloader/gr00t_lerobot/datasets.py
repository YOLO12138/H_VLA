...
from hvla.dataloader.gr00t_lerobot.video import get_all_frames, get_frames_by_timestamps

from hvla.dataloader.gr00t_lerobot.embodiment_tags import EmbodimentTag
from hvla.dataloader.gr00t_lerobot.schema import (
    DatasetMetadata,
    DatasetStatisticalValues,
    LeRobotModalityMetadata,
    LeRobotStateActionMetadata,
)
from hvla.dataloader.gr00t_lerobot.transform import ComposedModalityTransform
...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Pre-decoded frames root: if set, load JPEG/PNG files instead of decoding MP4.
        # Expected layout: {frames_root}/{camera_key}/episode_{ep:06d}/{step:06d}.jpg (RGB)
        #                  {frames_root}/{camera_key}/episode_{ep:06d}/{step:06d}.png (depth)
        ...
        self._parquet_cache: dict = {}  # trajectory_id → pd.DataFrame, pre-loaded at init
        ...
        # Pre-load all parquet files into memory (v2.0 only, ~14MB for 111 episodes).
        # Done before worker fork so workers share via copy-on-write.
    ...
    def _pack_sample(self, data: dict) -> dict:
        ...
        for video_key in self.modality_keys["video"]:
            image = data[video_key][0]  # (H,W,3) or (2H,W,3) uint8
            if "depth" in video_key:
                # Keep depth at original dual-row resolution.
                # collate_fn decodes uint8 dual-row → uint16 depth_mm → float32 XYZ,
                # then F.interpolate(..., mode="nearest") to ptmap_size.
                # BICUBIC resize here would corrupt the lo/hi byte boundary.
                img = Image.fromarray(image)
            else:
                # RGB: cv2.INTER_LINEAR is ~3-5x faster than PIL default BICUBIC
                resized = cv2.resize(image, (224, 224), interpolation=cv2.INTER_LINEAR)
                img = Image.fromarray(resized)
            if "wrist" not in video_key:
                prim_images.append(img)
            else:
                wrist_views.append(img)
    ...
    def _preload_parquet_cache(self):
        """Pre-load all trajectory parquet files into ``_parquet_cache``.

        Only applies to LeRobot v2.0 where each episode has its own parquet file.
        For v3.0 (multiple episodes per file), we keep the single-entry
        ``curr_traj_id`` cache to avoid loading large shared files.
        """
        if self._lerobot_version != "v2.0":
            return
        for traj_id in self._trajectory_ids:
            ...
            if parquet_path.exists():
                self._parquet_cache[traj_id] = pd.read_parquet(parquet_path)
        print(f"Pre-loaded {len(self._parquet_cache)} parquet files into memory")

    def get_trajectory_data(self, trajectory_id: int) -> pd.DataFrame:
        """Get the data for a trajectory."""
        if self._lerobot_version == "v2.0":
            return self._parquet_cache[trajectory_id]
        elif self._lerobot_version == "v3.0":
    ...
    def get_video(
        ...
        # ── Fast path: load pre-decoded JPEG/PNG files (no MP4 seek/decode) ──
        if self.frames_root is not None:
            return self._get_frames_from_disk(trajectory_id, key, step_indices)

        # ── Slow path: decode from MP4 via video backend ──────────────────────
    ...
    def _get_frames_from_disk(
        self,
        trajectory_id: int,
        camera_key: str,         # e.g. "head_rgb" (already stripped of "video." by get_video)
        step_indices: np.ndarray,  # shape (T,) — clamped step indices within episode
    ) -> np.ndarray:
        """Load pre-decoded frames from JPEG (RGB) or PNG (depth) files.

        File layout (written by predecode_dataset.py or build_lerobot_dataset_yam_v2.py):
          {frames_root}/{original_key}/episode_{ep:06d}/{step:06d}.jpg   (RGB)
          {frames_root}/{original_key}/episode_{ep:06d}/{step:06d}.png   (depth)
        where original_key = lerobot_modality_meta.video[camera_key].original_key
        e.g. "head_rgb" → "observation.images.head_rgb"

        Depth PNG formats supported:
          - uint16 native (mode I;16, shape H×W): from v2 builder — returned as-is
          - uint8 dual-row RGB (mode RGB, shape 2H×W×3): from predecode_dataset.py

        Returns:
            RGB:   np.ndarray shape (T, H, W, 3) uint8
            Depth: np.ndarray shape (T, H, W, 3) uint8 (dual-row, legacy)
                   OR list of PIL Images (for downstream _decode_lossless_depth_frame)
        """
        from PIL import Image as _Image

        # Map modality sub-key → actual lerobot storage key (same logic as get_video_path)
        original_key = self.lerobot_modality_meta.video[camera_key].original_key
        if original_key is None:
            original_key = camera_key

        ep_dir = self.frames_root / original_key / f"episode_{trajectory_id:06d}"
        is_depth = "depth" in camera_key

        frames = []
        for step_idx in step_indices:
            ext = ".png" if is_depth else ".jpg"
            frame_path = ep_dir / f"{int(step_idx):06d}{ext}"
            img = _Image.open(str(frame_path))
            if is_depth and img.mode == "I;16":
                # Native uint16 PNG — convert to uint8 dual-row on-the-fly
                # so downstream _decode_lossless_depth_frame works unchanged.
                depth_mm = np.array(img, dtype=np.uint16)  # (H, W)
                lo = (depth_mm & 0xFF).astype(np.uint8)
                hi = (depth_mm >> 8).astype(np.uint8)
                packed = np.vstack([lo, hi])  # (2H, W)
                frame_rgb = np.stack([packed, packed, packed], axis=-1)  # (2H, W, 3)
