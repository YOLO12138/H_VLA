...
DATASET_NAMED_MIXTURES = {
    ...
    # ---------------------------------------------------------------------------
    # AgiBot dual-arm humanoid datasets
    # Each entry is (dataset_dir_name, sampling_weight, robot_type).
    # dataset_dir_name is relative to datasets.vla_data.data_root_dir (AGIBOT_DATA_ROOT).
    # The builder writes one LeRobot dataset per task to --output-dir; the last
    # path component of --output-dir becomes the dataset_dir_name here.
    #
    # Example build command:
    #   python agibot/build_lerobot_dataset_agibot.py \
    #       --extracted-dir /tmp/agibot/extracted \
    #       --task-id 790 \
    #       --output-dir $AGIBOT_DATA_ROOT/lerobot_agibot
    #   → data_root_dir = $AGIBOT_DATA_ROOT, dataset_dir_name = "lerobot_agibot"
    # ---------------------------------------------------------------------------
    "agibot_debug": [
        ("lerobot_agibot", 1.0, "agibot"),   # single task dataset, for smoke-test / overfit
    ],
    "agibot": [
        ("lerobot_agibot", 1.0, "agibot"),   # add more task dirs here as they are built
    ],

    # ---------------------------------------------------------------------------
    # YAM dual-arm robot datasets (ZED stereo camera, lossless depth, 28D)
    # Each entry is (dataset_dir_name, sampling_weight, robot_type).
    # dataset_dir_name is relative to datasets.vla_data.data_root_dir (YAM_DATA_ROOT).
    #
    # Example build command:
    #   python yam/build_lerobot_dataset_yam.py \
    #       --raw-dir /tmp/yam/raw_episodes \
    #       --output-dir $YAM_DATA_ROOT/lerobot_yam
    #   → data_root_dir = $YAM_DATA_ROOT, dataset_dir_name = "lerobot_yam"
    # ---------------------------------------------------------------------------
    "yam_debug": [
        ("lerobot_yam", 1.0, "yam"),    # single task dataset, for smoke-test / overfit
    ],
    "yam_ai2": [
        ("lerobot_yam_ai2", 1.0, "yam"),  # AI2 YAM dataset (111 episodes)
    ],
    "yam": [
        ("lerobot_yam", 1.0, "yam"),    # add more task dirs here as they are built
    ],
    "place_lock": [
        ("place_lock_lerobot", 1.0, "yam"),  # data_root_dir/place_lock_lerobot
    ],
    # Sharded variants — same directory names, used with dataset_py: lerobot_datasets_yam_sharded
    # The v2 builder (build_lerobot_dataset_yam_v2.py) must have been run to populate frames/.
    "place_lock_sharded": [
        ("place_lock_lerobot", 1.0, "yam"),
    ],
    "yam_sharded": [
        ("lerobot_yam", 1.0, "yam"),
    ],
